...
class ModelCheckoutOrder extends Model
{
            ...
            foreach ($data['products'] as $product) {
                ...
                if (isset($product['article_id'])) {
                    if (isset($product['article'])) {
                        $this->db->query("INSERT INTO " . DB_PREFIX . "order_product SET order_id = '" . (int)$order_id . "', product_id = '" . (int)$product['product_id'] . "',article_id = '" . $this->db->escape($product['article_id']) . "', name = '" . $this->db->escape($product['name']) . "', model = '" . $this->db->escape($product['model']) . "', quantity = '" . (int)$product['quantity'] . "', price = '" . (float)$product['price'] . "', total = '" . (float)$product['total'] . "', tax = '" . (float)$product['tax'] . "', reward = '" . (int)$product['reward'] . "',article = '" .  $this->db->escape($product['article']) . "'");
                    } else {
                        $this->db->query("INSERT INTO " . DB_PREFIX . "order_product SET order_id = '" . (int)$order_id . "', product_id = '" . (int)$product['product_id'] . "',article_id = '" . $this->db->escape($product['article_id']) . "', name = '" . $this->db->escape($product['name']) . "', model = '" . $this->db->escape($product['model']) . "', quantity = '" . (int)$product['quantity'] . "', price = '" . (float)$product['price'] . "', total = '" . (float)$product['total'] . "', tax = '" . (float)$product['tax'] . "', reward = '" . (int)$product['reward'] . "'");
                    }
    ...
    public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false, $override = false)
    {
        ...
        if ($order_info['order_status_id'] && $order_status_id && $notify) {
          if (strpos($order_info['payment_code'], 'iwiw') && $this->config->get($order_info['payment_code'].'_status')){
                if ($this->config->get($order_info['payment_code'].'_on_status_id') == $order_status_id){
                  ...
                  $merchant_url = $this->model_extension_payment_qiwiw->getCustomFields($order_info, 'href');
        ...
        if ($order_info) {
            ...
            if (!$order_info['order_status_id'] && $order_status_id) {
                ...
                $data['text_footer'] = $language->get('text_new_footer1') . '<br><br>';
                $data['text_footer'] .= $language->get('text_new_footer2') . '<br><br>';
                $data['text_footer'] .= $language->get('text_new_footer3') . '<br>';
                $data['text_footer'] .= $language->get('text_new_footer4') . '<br>';
                $data['text_footer'] .= $language->get('text_new_footer5') . '<br>';
                $data['text_footer'] .= $language->get('text_new_footer6') . '<br>';
                $data['text_footer'] .= $language->get('text_new_footer7') . '<br>';
                ...
                $text .= $language->get('text_new_footer1') . "\n\n";
                $text .= $language->get('text_new_footer2') . "\n\n";
                $text .= $language->get('text_new_footer3') . "\n";
                $text .= $language->get('text_new_footer4') . "\n";
                $text .= $language->get('text_new_footer5') . "\n";
                $text .= $language->get('text_new_footer6') . "\n";
                $text .= $language->get('text_new_footer7') . "\n";
