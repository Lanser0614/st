...
class ControllerExtensionModuleFX extends Controller {
    ...
    public function index($pre_data = array()) {
		...
		if (($route == 'product/category') && !empty($this->request->get['manufacturer_id']) && !empty($this->request->get['path'])) {
			...
			$this->load->model('module/fx');
			$cid = explode("_", $this->request->get['path']);
			$category_id = (int)array_pop($cid);
			
			$m_name = $this->model_module_fx->getManufacturer($mid);
		...
		if (!empty($out['canonical'])) {
		    $hrefCanonical = $out['canonical'];

            if(strpos($hrefCanonical, "?page=") !== false) {
                $hrefCanonical = substr($hrefCanonical, 0, strpos($hrefCanonical, "?page="));
            } elseif (strpos($hrefCanonical, "page=") !== false){
                $hrefCanonical = substr($hrefCanonical, 0, strpos($hrefCanonical, "page="));
            }

			if ($this->cicleProtect($out['canonical'])) $this->document->addLink($hrefCanonical, 'canonical');
			
			if ($out['canonical'] != $this->full_url()) $out['clear'] = $this->config->get('fx_description_canonical') ? 1 : $out['clear'];
		}
		
		if ($out['clear']) $out['data']['description'] = $out['data']['description_2'] = $out['data']['thumb'] = $out['data']['des2'] = $out['data']['ext_description'] = $out['data']['description_bottom'] = '';
	...
	public function test404() {	
		...
		while (!empty($parts)) {
			...
			$query = $this->model_module_fx->getQuery($last);
	...
	public function presetData($mod_data = array()) {
		...
		if(isset($mod_data['data']['limit']) && $mod_data['data']['limit'] == 25){
            $mod_data['total'] = ceil($mod_data['total'] / 25);
        } else{
	...
	public function data() {
		...
		$this->load->model('module/fx');
		$data = $this->model_module_fx->data();
		return $data;
    }
	...
	public function testManuficturer($mid = 0) {
		...
		if (($route == 'product/category') && isset($this->request->get['manufacturer_id'])) {
				...
				$this->load->model('module/fx');
				$man['name'] = $this->model_module_fx->getManufacturer($mid);
    ...
    public function test() {
		...
		$category_id = $this->model_module_fx->getCat();
    ...
    public function scan($encode=true) {
		...
		$json['scan'] = $this->model_module_fx->scanData('fx');
		
		$json['debug'] = $this->model_module_fx->debugData('fx');
    ...
    public function debug() {
		...
		$this->load->model('module/fx');
		$data = $this->model_module_fx->debugData('fx');
