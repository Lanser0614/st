    ...
    public function saveAccount($post)
    {
        ...
        if (!$this->customer->isLogged()) {
            ...
            if ($this->validate($post)) {
                ...
                $customer['custom_fields'] = $this->model_account_custom_field->getCustomFields($this->config->get('config_customer_group_id'));
    ...
    public function voucherthemes()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'GET') {
            ...
            if ($this->opencartVersion < 2100) {
                ...
                $this->json["data"] = $this->model_checkout_voucher_theme->getVoucherThemes();

            } elseif ($this->opencartVersion < 2300 && $this->opencartVersion >= 2100) {
                $this->load->model('total/voucher_theme');
                $this->json["data"] = $this->model_total_voucher_theme->getVoucherThemes();
            } else {
