...
class ControllerInformationNews extends Controller {
		...
		if ($news_list) {
			...
			foreach ($news_list as $result) {
				...
				$data['news_list'][] = array(
					...
					'description' => (utf8_strlen($descr) > $news_setting['description_limit'] ? utf8_substr($descr, 0, $news_setting['description_limit']) . '...' : $descr),
	...
	public function info(){
		...
		if ($news_info) {
			...
			if ($news_info['image']) {
				$data['image'] = $this->model_tool_image->resize($news_info['image'], $news_setting['news_popup_width'], $news_setting['news_popup_height']);
			} else {
				$data['image'] = false;
			}
