...
//            echo '<pre>';
//            print_r($price_autopiter);die();
//            $product_info['price'] = $price_autopiter;
//            $this->model_catalog_product->editProduct($product_id,$price_autopiter);
//        }
...
////            $product_info['price'] = $this->cache->get('price_article'); //забираем в массив уже готовые данные и не делаем запросов
//        }else{
        try {
            $autopiter = new AutoPiterComponent($product_info['model']);
...
//        }
        ...
        if ($product_info) {
            ...
            $data['min_price_analog'] = $this->currency->format($this->tax->calculate($product_info['min_price_analog'], $product_info['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency']);
            ...
            $text = '';
            if (!empty($attribute_groups) && isset($attribute_groups[0]) && isset($attribute_groups[0]['attribute'])) {
                foreach ($attribute_groups[0]['attribute'] as $attribute) {
                    if ($attribute['name'] == 'Страна-изготовитель') {
                        $text = '. Страна изготовления - ' . $attribute['text'];
                    }
                }
            }
            $price_seo = $this->currency->format($this->tax->calculate($product_info['price'], $product_info['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency']);
            ...
            if ($product_info['meta_h1']) {
                $data['heading_title1'] = mb_strtolower($product_info['meta_h1']);
            } else {
                $data['heading_title1'] = mb_strtolower($product_info['name']);
            }

            if ($product_info['meta_title']) {
                $this->document->setTitle($product_info['meta_title']);
            } else {
                $title = $data['heading_title'] . ' - купить в Москве по цене ' . $price_seo . '. рублей в интернет магазине ST-Spares';
                $this->document->setTitle($title);
            }

            if (empty($product_info['meta_description'])) {
                $description = $data['heading_title'] . ': 💰 цены, 📑 описание, 👍 отзывы покупателей 📓 Гарантия производителя! 🚚 Доставляем по Москве и Московской области! Звоните ☎ +7 (495) 989-43-31 +7 (800) 775-70-72 +7 (985) 310-95-50';
                $this->document->setDescription($description);
            } else {
                $this->document->setDescription($product_info['meta_description'] . ': 💰 цены, 📑 описание, 👍 отзывы покупателей 📓 Гарантия производителя! 🚚 Доставляем по Москве и Московской области! Звоните ☎ +7 (495) 989-43-31 +7 (800) 775-70-72 +7 (985) 310-95-50');
            }
            #endregion
            ...
            $data['manufacturer_link'] = $this->url->link('product/manufacturer/info', 'manufacturer_id=' . $product_info['manufacturer_id']);

            $manufacturer_info = $this->model_catalog_manufacturer->getManufacturer($product_info['manufacturer_id']);
            if ($manufacturer_info) {
                if ($manufacturer_info['image']) $data['manufacturer_thumb'] = $this->model_tool_image->resize($manufacturer_info['image'], 80, 60);
                else $data['manufacturer_thumb'] = false;
            }

            /*
                        if ($product_info['quantity'] <= 0) {
                            $data['stock'] = '<div class="stock nostock">'.$product_info['stock_status'];
                        } else {
                            $data['stock'] = '<div class="stock instock">'.$this->language->get('text_instock');
                        }
                        if ($this->config->get('config_stock_display')) $data['stock'] .= ' <b>('.$product_info['quantity'].')</b>';

                        $data['stock'] .= '</div>';
            */
            $data['stock'] = '<div class="stock ';
            if ($product_info['stock_status_id'] == 7) $data['stock'] .= 'instock';
            ...
            $data['autocalc_option_special'] = 0;
            $data['autocalc_option_discount'] = 1;
            //
            ...
            foreach ($results as $result) {
                ...
                if ($result['quantity'] <= 0) {
                    ...
                    $stock = '<span class="instock">' . $this->language->get('text_instock') . '</span>';
                }
        ...
        else 
        {
            ...
            #region Getting Related Products version 2
            $data['products'] = array();
            $limit = 6;
            $filter_data = array(
