...
{
   $query =  $this->db->query("SELECT * FROM manufacturer_description WHERE manufacturer_id = " . $id);
   ...
   return $query->rows;
    ...
    public function getProductsByAlias($alias, $product_ids, $customer, $sortSql = "ORDER BY p.product_id ASC")
    {
        ...
        foreach($product_id->rows as $key){
            $key["SUBSTR(query, 12, 5)"];
            $product_ids = intval($key["SUBSTR(query, 12, 5)"]);
        }
        ...
        (SELECT price FROM " . DB_PREFIX . "product_discount pd2 WHERE pd2.product_id = p.product_id AND pd2.customer_group_id = '" . (int)$customer_group_id . "' AND pd2.quantity = '1' AND ((pd2.date_start = '0000-00-00' OR pd2.date_start < NOW()) AND (pd2.date_end = '0000-00-00' OR pd2.date_end > NOW())) ORDER BY pd2.priority ASC, pd2.price ASC LIMIT 1) AS discount, (SELECT price FROM " . DB_PREFIX . "product_special ps WHERE ps.product_id = p.product_id AND ps.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < NOW()) AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW())) ORDER BY ps.priority ASC, ps.price ASC LIMIT 1) AS special, (SELECT product_special_id FROM " . DB_PREFIX . "product_special ps WHERE ps.product_id = p.product_id AND ps.customer_group_id = '" . (int)$customer_group_id . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < NOW()) AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW())) ORDER BY ps.priority ASC, ps.price ASC LIMIT 1) AS special_id, (SELECT points FROM " . DB_PREFIX . "product_reward pr WHERE pr.product_id = p.product_id AND customer_group_id = '" . (int)$customer_group_id . "') AS reward, (SELECT ss.name FROM " . DB_PREFIX . "stock_status ss WHERE ss.stock_status_id = p.stock_status_id AND ss.language_id = '" . (int)$this->config->get('config_language_id') . "') AS stock_status, (SELECT wcd.unit FROM " . DB_PREFIX . "weight_class_description wcd WHERE p.weight_class_id = wcd.weight_class_id AND wcd.language_id = '" . (int)$this->config->get('config_language_id') . "') AS weight_class, (SELECT lcd.unit FROM " . DB_PREFIX . "length_class_description lcd WHERE p.length_class_id = lcd.length_class_id AND lcd.language_id = '" . (int)$this->config->get('config_language_id') . "') AS length_class, (SELECT AVG(rating) AS total FROM " . DB_PREFIX . "review r1 WHERE r1.product_id = p.product_id AND r1.status = '1' GROUP BY r1.product_id) AS rating, (SELECT COUNT(*) AS total FROM " . DB_PREFIX . "review r2 WHERE r2.product_id = p.product_id AND r2.status = '1' GROUP BY r2.product_id) AS reviews, p.sort_order FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) LEFT JOIN " . DB_PREFIX . "manufacturer m ON (p.manufacturer_id = m.manufacturer_id) WHERE p.product_id IN (" . implode(',', $product_ids) . ") AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p.status = '1' AND p.date_available <= NOW() AND
        p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'" . $sortSql);
        if ($query->num_rows) {
    ...
    public function getProductWithAlias($alias){
        $product_id = $this->db->query("SELECT SUBSTR(query, 12, 5) FROM `url_alias` WHERE `keyword` = '$alias'");
        foreach($product_id->rows as $key){
            $key["SUBSTR(query, 12, 5)"];
            $key_value = intval($key["SUBSTR(query, 12, 5)"]);
        }
       // var_dump($key_value);
       ...
       (int)$key_value . "' AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'");
      ...
      //  $product_id = $this->db->query("SELECT SUBSTR(query, 12, 5) FROM `url_alias` WHERE `keyword` LIKE '%$alias%'");
        ...
        if ($query->num_rows) {
            return array(
                ...
                'name' => $query->row['name'],
                'min_price_analog' => $query->row['min_price_analog'],
    ...
    // public function getChecksum() { e $query = $this->db->query("CHECKSUM TABLE " . DB_PREFIX . "product, " . DB_PREFIX . "category," . DB_PREFIX . "product_to_category," . DB_PREFIX . "product_description" ); return $query->rows; }
    ...
    public function getProductsAllData($data = array(), $customer)
    {
        ...
        //     var_dump('ok');
        // }
        $sql = " SELECT DISTINCT *, pd.name AS name, p.image, m.name AS manufacturer, (SELECT price FROM " . DB_PREFIX . "product_discount pd2 WHERE pd2.product_id = p.product_id AND pd2.customer_group_id = '" . (int)$customer_group_id . "' AND pd2.quantity = '1' AND ((pd2.date_start = '0000-00-00' OR pd2.date_start < NOW()) AND (pd2.date_end = '0000-00-00' OR pd2.date_end > NOW())) ORDER BY pd2.priority ASC, pd2.price ASC LIMIT 1) AS discount, (SELECT price FROM " . DB_PREFIX . "product_special ps WHERE ps.product_id = p.product_id AND ps.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < NOW()) AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW())) ORDER BY ps.priority ASC, ps.price ASC LIMIT 1) AS special, (SELECT product_special_id FROM " . DB_PREFIX . "product_special ps WHERE ps.product_id = p.product_id AND ps.customer_group_id = '" . (int)$customer_group_id . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < NOW()) AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW())) ORDER BY ps.priority ASC, ps.price ASC LIMIT 1) AS special_id, (SELECT points FROM " . DB_PREFIX . "product_reward pr WHERE pr.product_id = p.product_id AND customer_group_id = '" . (int)$customer_group_id . "') AS reward, (SELECT ss.name FROM " . DB_PREFIX . "stock_status ss WHERE ss.stock_status_id = p.stock_status_id AND ss.language_id = '" . (int)$this->config->get('config_language_id') . "') AS stock_status, (SELECT wcd.unit FROM " . DB_PREFIX . "weight_class_description wcd WHERE p.weight_class_id = wcd.weight_class_id AND wcd.language_id = '" . (int)$this->config->get('config_language_id') . "') AS weight_class, (SELECT lcd.unit FROM " . DB_PREFIX . "length_class_description lcd WHERE p.length_class_id = lcd.length_class_id AND lcd.language_id = '" . (int)$this->config->get('config_language_id') . "') AS length_class, (SELECT AVG(rating) AS total FROM " . DB_PREFIX . "review r1 WHERE r1.product_id = p.product_id AND r1.status = '1' GROUP BY r1.product_id) AS rating, (SELECT COUNT(*) AS total FROM " . DB_PREFIX . "review r2 WHERE r2.product_id = p.product_id AND r2.status = '1' GROUP BY r2.product_id) AS reviews, p.sort_order ";
    ...
    { 
        ...
        $alias = $this->db->query("SELECT  * FROM url_alias WHERE query = 'product_id=$product_id'");
    ...
    private function filterSql($data, $onlycount = false)
    {
        ...
        if (!empty($data['filter_category_id'])) {
            if (!empty($data['filter_sub_category'])) {
                ...
                if (count($categoryImplode) > 1) {
                    ...
                    // $id = ("SELECT * FROM product_to_category inner JOIN category on product_to_category.category_id = category.category_id AND category.parent_id = 59");
                    // var_dump($id);
                }
            }
            if (!empty($data['filter_filter'])) {
        ...
        if (empty($onlycount)) {
            ...
            $sort_data = array('name' => 'pd.name', 'model' => 'p.model', 'quantity' => 'p.quantity', 'price' => 'p.price', 'id' => 'p.product_id', 'rating' => 'rating', 'sort_order' => 'p.sort_order', 'date_added' => 'p.date_added', 'date_modified' => 'p.date_modified');
    ...
    public static $fields = array("category" => "p2c.category_id", "quantity" => "p.quantity", "stock_status" => "p.stock_status_id", "manufacturer" => "p.manufacturer_id", "model" => "p.model", "upc" => "p.upc", "name" => "pd.name", "date_start" => "ps.date_start", "date_end" => "ps.date_end", "product_id" => "p.product_id", "price" => "p.price", "status" => "p.status", "date_available" => "p.date_available", "store_id" => "p2s.store_id", "filter_text" => "filter_text",);
    public static $operands = array("=" => "=", "!=" => "!=", ">" => ">", ">=" => ">=", "<" => "<", "<=" => "<=", "!<" => "!<", "!>" => "!>", "<>" => "<>", "in" => "in", "not_in" => "not in", "like" => "like",);
    public static $logicalOperands = array("and" => "AND", "or" => "OR");
    public static $sort_data = array('name' => 'pd.name', 'model' => 'p.model', 'quantity' => 'p.quantity', 'price' => 'p.price', 'rating' => 'rating', 'sort_order' => 'p.sort_order', 'date_added' => 'p.date_added');
            ...
            if (array_key_exists($filter['field'], static::$fields)) {
                ...
                if (static::$fields[$filter['field']] == 'filter_text') {
                    ...
                    if ($operand == "in" || $operand == "not in") {
                        $sql .= " " . $logicalOperand . " " . static::$fields[$filter['field']] . " " . $operand . " (" . $this->db->escape(implode(",", $filter['value'])) . ")";
                    } elseif ($operand == "like") {
                        $sql .= " " . $logicalOperand . " " . static::$fields[$filter['field']];
                        $sql .= " LIKE '%" . $this->db->escape(strtolower($filter['value'])) . "%'";
                    } else {
                        $sql .= " " . $logicalOperand . " " . static::$fields[$filter['field']] . " " . $operand . " " . $this->db->escape($filter['value']);
                    }
                }
            }
        }
    ...
    public function getProducts($data = array())
    {
        ...
        $sql .= " LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'" . $where;
        ...
        if (!empty($data['filter_name']) || !empty($data['filter_tag'])) {
            ...
            //				$implode = array();
            //
            //				$explode = explode(' ', trim(preg_replace('/\s+/', ' ', $data['filter_name'])));
            //
            //                foreach ($explode as $word) {
            //                    $filterSearch = $this->filterSearch($word);
            //                    if(!empty($filterSearch)){
            //                        foreach($filterSearch as $filterElement){
            //                            $implode[] = "pd.name LIKE '%" . $this->db->escape($filterElement) . "%'";
            //                        }
            //                    }else{
            //                        $implode[] = "pd.name LIKE '%" . $this->db->escape($word) . "%'";
            //                    }
    ...
    {
        $txt = [];
        $q_count = mb_strlen($search);
        $k = 3;
        for ($i = 2; $i < $q_count; $i++) {
            $txt[] = mb_substr($search, 0, $k);
            $k++;
        }
        return $txt;
    }
    ...
    public function editProduct($product_id, $price)
    {
        $this->db->query("UPDATE " . DB_PREFIX . "product SET price = " . (float)$price . " WHERE product_id = '" . (int)$product_id . "'");
    }

    public function editPriceAnalog($product_id, $price)
    {
        $this->db->query("UPDATE " . DB_PREFIX . "product SET min_price_analog = " . (float)$price . " WHERE product_id = '" . (int)$product_id . "'");
    }
}
