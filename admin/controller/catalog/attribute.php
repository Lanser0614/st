	...
	public function autocomplete() {
		...
		if (isset($this->request->get['filter_name'])) {
			...
			foreach ($results as $result) {
				...
				$resultsallattr = array();
        $resultsallattr = $this->model_catalog_attribute->getalldataAttributes($result['attribute_id']);
        $data_values = array();
        foreach ($resultsallattr as $data_value){
            $data_values[] = array(
               'text'            => strip_tags(html_entity_decode($data_value['text'], ENT_QUOTES, 'UTF-8')),
               'langid'          => $data_value['language_id']
            );
        }
										
				$json[] = array(
				  'attributevalues' => $data_values,
					'attribute_id'    => $result['attribute_id'],
